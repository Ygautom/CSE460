...
public class SmartCityHubApp {
    ...
    public static void main(String[] args) {
        ...
        // NEW: Authentication (Login / Register Account use cases)
        ...
        auth.login("alice@email.com", "pw");        // valid
        auth.login("alice@email.com", "wrongpw");    // invalid
        System.out.println();
        ...
        // NEW: typed request built by ServiceRequestFactory
        ServiceRequest request = alice.submitTypedRequest(
                "POTHOLE",
                "R1",
                "Pothole on Main Street");
        requestManager.addRequest(request);

        // NEW: manager assigns the task before working it
        manager.assignTask(request);
        ...
        ServiceRequest request2 = alice.submitTypedRequest(
                "BROKEN_STREETLIGHT",
                "R2",
                "Broken Streetlight");
        ...
        // NEW: review all requests, filter by status
        System.out.println("----- Review Service Requests -----");
        requestManager.displayAllRequests();
        requestManager.displayByStatus(RequestStatus.RESOLVED);

        System.out.println();

        // NEW: reports (Generate Reports use case)
        System.out.println("----- Reports -----");
        manager.generateReport(eventManager);
